...
public class a extends Applet implements Runnable {
	...
	public void run() {
		...
		BufferedImage image = new BufferedImage(256, 208, 1);
		...
		while (true) {
			...
			if (gameState != GAME_STATE_ENTRY && draculaState != DRACULA_STATE_HIDDEN) {
				// draw dracula
				g.translate(draculaX, 128);
				if (draculaReversed) {
					g.scale(-1, 1);
				}

				g.drawImage(sprites[SPRITE_DRACULA_2], 0, 0, null);
				g.drawImage(sprites[SPRITE_DRACULA_3], -6, -10, null);
				g.drawImage(sprites[SPRITE_DRACULA_5], -8, 0, null);
				if (draculaOpened) {
					g.drawImage(sprites[SPRITE_DRACULA_4], -8, 8, null);
					g.drawImage(sprites[SPRITE_DRACULA_7], -24, 0, null);
					g.drawImage(sprites[SPRITE_DRACULA_8], -16, 0, null);
					g.fillRect(-16, 6, 8, 42);
				} else {
					g.drawImage(sprites[SPRITE_DRACULA_1], -16, 0, null);
					g.drawImage(sprites[SPRITE_DRACULA_6], -8, 44, null);
				}
				if (draculaReversed) {
					g.translate(1, 0);
				}
				g.drawLine(7, 16, 7, 47);
				g.drawLine(0, 47, 7, 47);
				if (draculaOpened) {
					g.drawLine(-13, 0, -9, 0);
				}

				if (draculaState == DRACULA_STATE_FADE_IN || draculaState == DRACULA_STATE_DEAD) {
					g.setColor(new Color(0xFFFFFF | ((draculaState == DRACULA_STATE_FADE_IN ? draculaCounter : (draculaCounter >> 1)) << 24), true));
					g.fillRect(-20, -10, 32, 60);
					g.setColor(Color.BLACK);
				}

				if (gameState == GAME_STATE_RISING || firstFadeIn) {
					// draw floating head
					g.drawImage(sprites[SPRITE_DRACULA_3], -7, headY, null);
				}
			...
			// draw subweapon box
			g.drawRect(128, 8, 31, 21);
			g.drawRect(129, 9, 29, 19);
			g.rotate(1.1f);
			g.drawImage(sprites[SPRITE_CROSS], 74, -127, null);
			g.setTransform(affineTransform);

			// draw power bars
			for (i = 0; i < 16; i++) {
				if (playerPower > i) {
					g.fillRect(56 + (i << 2), 9, 3, 6);
				} else {
					g.drawRect(56 + (i << 2), 9, 2, 5);
				}
				if (((draculaPower + 1) >> 1) > i) {
					g.fillRect(56 + (i << 2), 17, 3, 6);
				} else {
					g.drawRect(56 + (i << 2), 17, 2, 5);
				}
			}

			// draw triple
			g.drawImage(sprites[SPRITE_TRIPLE], 208, 8, null);

			// draw floor
			for (i = 0; i < 16; i++) {
				g.drawImage(sprites[SPRITE_BLOCK], i << 4, 176, null);
				g.drawImage(sprites[SPRITE_BLOCK], i << 4, 192, null);
			}

			// draw player
			g.translate(playerX + 8, playerY);
			if (playerReversed) {
				g.scale(-1, 1);
			}
			...
			if (playerDead < 96 && playerDead > 8) {
				g.drawImage(sprites[SPRITE_DEAD_1], -16, 15, null);
				g.drawImage(sprites[SPRITE_DEAD_2], 0, 15, null);
			} else if (playerDead < 96 && playerDead > 0) {
				g.drawImage(sprites[SPRITE_BODY_1], -8, 7, null);
				g.drawImage(sprites[SPRITE_KNEELING], -8, 23, null);
			} else if (playerHurt) {
				g.drawImage(sprites[SPRITE_PLAYER_HURT], -8, 0, null);
			} else if (playerWhipping > 0) {
				if (playerKneeling || (playerJumping && playerVy < 0)) {
					g.drawImage(sprites2[SPRITE_KNEELING], -8, 23, null);
					g.translate(0, 7);
				} else {
					g.drawImage(sprites2[SPRITE_LEGS_1], -8, 16, null);
				}
				if (playerWhipping > WHIP_BOWED) {
					if (!playerThrowing) {
						g.drawImage(sprites[SPRITE_WHIP_HANGING], -24, 7, null);
					}
					g.drawImage(sprites2[SPRITE_BODY_4], -16, 1, null);
				} else if (playerWhipping > WHIP_EXTENDED) {
					if (!playerThrowing) {
						g.drawImage(sprites[SPRITE_WHIP_BOWED], -24, 4, null);
					}
					g.drawImage(sprites2[SPRITE_BODY_5], -8, 0, null);
				} else {
					g.drawImage(sprites2[SPRITE_BODY_6], -8, 2, null);
					g.drawImage(sprites2[SPRITE_BODY_7], 8, 10, null);
					if (!playerThrowing) {
						// draw extended whip
						for (i = 0; i < 4; i++) {
							g.drawImage(sprites[SPRITE_WHIP_MIDDLE], 12 + (i << 3), 12, null);
						}
						g.drawImage(sprites[SPRITE_WHIP_END], 44, 10, null);
					}
				}
				if (playerKneeling) {
					g.translate(0, -7);
				...
				if (playerKneeling || (playerJumping && playerVy < 0)) {
					g.drawImage(sprites2[SPRITE_BODY_1], -8, 7, null);
					g.drawImage(sprites2[SPRITE_KNEELING], -8, 23, null);
				} else {
					i = playerJumping ? 0 : (playerWalkIndex == 3 ? 1 : playerWalkIndex);
					g.drawImage(sprites2[SPRITE_BODY_1 + i], -8, 0, null);
					g.drawImage(sprites2[SPRITE_LEGS_1 + i], -8, 16, null);
				}
			}
			g.setTransform(affineTransform);
			...
			for (i = 0; i < queue.size(); i++) {
				...
				if (object[OBJ_SPRITE] >= 0) {
					g.translate((int) object[OBJ_X], (int) object[OBJ_Y]);
					...
					g.drawImage(sprites[(int) object[OBJ_SPRITE]], (int) object[OBJ_DX], (int) object[OBJ_DY], null);
					g.setTransform(affineTransform);
				}
			}
			...
			// draw fade
			if (playerDead > 64) {
				g.setColor(new Color(((playerDead >= 96) ? (127 - playerDead) : (playerDead - 64)) << 27, true));
			...
			if (g2 == null) {
				...
				g2.drawImage(image, 0, 0, 512, 416, null);
			}
